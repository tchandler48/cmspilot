...
int test( condex *cond )
{
  ...
  if( (cond->op == OP_YES) && (get_nv("#MATCHED") == TRUE) ) {
    return TRUE;
  } else if( (cond->op == OP_NO) && (get_nv("#MATCHED")==FALSE) ) {
    return TRUE;
...
void init()
{
  ...
  nvar->next = (struct numvar *)new_numvar( "#RPILOT", 14 );
  svar->next = (struct strvar *)new_strvar( "$PROMPT", ">" );
...
{
  line *lne = new_line( trim(get_stvl(str)), ' ', -1 );
